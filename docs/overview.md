...
- 小包场景吞吐 **28K~35K** ops/s，是 Spring MVC 的 **1.6~2.1x**
- P50 延迟 **0.11~0.14ms**，约为 Spring MVC 的 **50%**
- 稳态堆占用 **15MB**，约为 Spring MVC 的 **75%**
- SSE 流式场景吞吐 **1,546** ops/s，达到 Tomcat 的 **4.51x**
...
本框架允许请求直接在 Netty EventLoop 上执行，同时提供 `@RunInPool` 注解让业务方按方法粒度决定是否需要切换到业务线程池。三种编程模型自由选择：

- **同步阻塞**：`@RunInPool` + 业务线程池，和传统 Servlet 模型相同，但可按方法粒度控制
- **EventLoop 直处理**：无 `@RunInPool`，适合纯 CPU 计算或配合响应式驱动（R2DBC、Reactive Redis）
- **未来虚拟线程**：JDK 21+ 虚拟线程天然兼容，`@RunInPool` 不再需要
...
然而 SSE 在传统 Servlet 容器上性能表现不佳——Tomcat 的 SSE 吞吐仅约 **342 ops/s**，成为 AI 应用链路的瓶颈。本项目的 SSE 吞吐达到 **1,546 ops/s**，是 Tomcat 的 **4.51x**。支撑这一性能的是 **NettyStreamSender** 的无锁 Drain Loop 设计：写入操作不依赖线程池调度，直接在 EventLoop 上完成批量刷新，避免传统 Servlet 容器中 SSE 连接独占线程的问题。
...
| **高吞吐 API 服务** | 28K~35K ops/s 的吞吐能力，适合接口层卸载 |
| **延迟敏感业务** | P50 0.11~0.14ms，是 Spring MVC 的 50% |
| **SSE / 流式推送** | 无锁 Drain Loop 设计，吞吐达 Tomcat 的 4.51x |
